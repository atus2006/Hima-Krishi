// Generate a QR, upload to transfer.sh, and send via Twilio WhatsApp
...
(async () => {
  try {
    ...
    const farmerId = 'qr-temp-' + Date.now();
    ...
    console.log('Uploading to transfer.sh as', filename, '(public, temporary)');
    ...
    console.error('Error sending QR:', err && err.message ? err.message : err);
